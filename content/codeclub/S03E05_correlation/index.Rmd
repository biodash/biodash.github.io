...
Now let select variables from penguins using the pipe operator `%>%` and the
`select()` function.
We then specify name of the variables that we want to select.
...
We can apply the `shave()` function remove the top triangle:
...
You can see that values of upper triangle are replaced by `NA`s. 
