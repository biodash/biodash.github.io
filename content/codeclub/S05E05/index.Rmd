...
_(Note that R Markdown is putting `#>` in front of each line, this is not part of the file.)_
...
When you don't specify the column types, as above, _readr infers_ them:
...
In the above example, we simply provided a file name.
This code therefore assumes that the file is present in your current R
"working directory".
^[Which should in fact be the case when you run all the code on this page:
the `download.file` function similarly downloaded the file to your
working directory, because we only gave that function a file name too.]
If the file is located elsewhere, that code will fail:
_readr_ will _not_ look for a file with this name across your entire computer.
...
- Change your working directory with `setwd()` (_not_ recommended)
- Include the _location_ of the file when you call the function
...
and paste that location into R.
...
then backslashes will also work in R.) 
...
and to clean up the column names with `clean_names()`.
...
Bonus <sup>bonus</sup>: There are two sheets, `Sheet1` and `Sheet2`.
