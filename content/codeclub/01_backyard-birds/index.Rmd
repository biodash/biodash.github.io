...
and we will therefore automatically have as our working directory
the directory that contains the `.Rproj` file.
...
dir()      # Listing files in your current directory.
           # This should return at least the `.RProj` file.
```
...
- Explore the output of the following commands, and try to understand what you see.
  What does a single row represent, and what is in each column?

```{r, eval = FALSE}
birds          # Type the name of an object to print it to screen
...
View(birds)    # In RStudio, will open object in a table in an Rstudio tab.
               # Or: clicking on the object in the `Environment` pane!
```
...
- What are some questions you would like to explore with this dataset?
  ...
  to go ahead and try to answer one or more of these questions already.
...
Note that we read in our file specifying any additional arguments to the
`readr()` function,
...
- There are also arguments to these functions for when you need to skip lines,
  ...
  my_df <- read_delim(
    file = 'file.txt',
    sep = '\t',               # Manually specify tab as delimiter
