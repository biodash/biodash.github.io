...
(For future Code Club sessions, OSC users will have the Project automatically opened upon opening RStudio Server. If you're working locally and also using RStudio outside of this Project, you can open it by `File` > `Open Project` inside RStudio, or by clicking the `.Rproj` file in your file browser,
...
because our newly created Project is open,
and we will therefore automatically be in the directory that contains the `.Rproj` file.
...
dir()      # Listing files in your current directory.
           # This should return the `.RProj` file.
```
...
```{r, eval=TRUE}
# install.packages("tidyverse") # If you're working locally and did not install it yet

library(tidyverse)    # library() is the standard function to load R packages
```

If it worked, you should get the same output as shown in the code block above:
...
for data analysis, which we will be using a lot in Code Club (despite some
...
```{r}
# Just printing the glimpse() output which will show the number of rows and columns:
glimpse(birds)
```

```{r}
# You can also check the number of rows and columns directly using:
dim(birds)          # Will return "number_of_rows, number_of_columns"
...
birds2_file_url <- 'https://raw.githubusercontent.com/biodash/biodash.github.io/master/assets/data/birds/birds_read-challenge.txt'
birds2_file <- 'data/birds/birds_read-challenge.txt'
...
- Note that you can also use a succinct column type specification like
  ...
  type e.g. `?read_tsv` for details in the help file.
...
birds2 <- read_csv(     # `read_csv()` because the file is comma-delimited
  file = birds2_file,
  skip = 1,             # The first line is not part of the data frame
  comment = '$',        # Line 228 is a comment that starts with a `$`.
  col_types = 'fcdiii'  # 'f' for factor, 'c' for character,
                        # ... 'd' for double (=numeric), 'i' for integer.
  )
...
birds2 <- read_csv(
  ...
  skip = 1,
  comment = '$',
  col_types = cols(        # Note that we can omit columns for which we
...
Finally, you can read excel files directly using the *readxl* package,
Google Sheets directly from the web using the *googlesheets4* package,
and non-tabular data using the base R `readLines()` function.
