...
When making plots, we typically specify what should go along the y-axis, too.
...
To explore variation in a _continuous_ variable like `carat`,
...
If we want to show the variation for different levels of `cut` separately
in a single graph,
...
yet no visible bars on the sides of the plot:
...
You could also use `ylim()` from the equivalent `ylim()` / `xlim()` pair of
functions,
...
In that case, you can convert outliers to `NA`s (missing values),
