...
# If you have the tidyverse installed, you only need to load it:
library(tidyverse)
```

We'll talk about formatting plots to your heart's delight in Chapter 12,
but one trick for now is to set a ggplot2 plotting "theme" that differs from
the default for the rest of your R session:
...
- If we map the `iris` dataframe variable `Petal.Length` to the `x`-axis aesthetic,
  then the x-axis of our plot will represent the values from the column
  `Petal.Length` in the dataframe.

- If we map the variable `Species` to the `color` aesthetic,
...
Instead, the key difference is that we are not specifying it insides an
`aes()` function call.
...
along with summaries of the data such as a trend line (`geom_smooth()`) or
a box plot (`geom_boxplot()`).

### Local vs global aesthetics

One thing that's important to realize once you start using multiple geoms in
a single plot is the distinction between "global" versus "local" aesthetics:
...
Compare this with the plot that is created when we define the `color` mapping
_locally_, and _only_ for `geom_point()`:
...
to compare the distributions of a variable among groups.
...
- Very similar to our `iris` plot above,
  make a ridgeline plot of mileage (`hwy`) for cars with different drive trains
  (`drive`).
  And like in our `iris` plot,
  also map the `fill` aesthetic to the categorical variable (`drive`).
...
- When we map the `color` aesthetic,
  the **line** (as opposed to the _fill_) around the density shapes will
...
- Going back to the initial erroneous code:
  does _ggplot_'s behavior make sense to you,
  or would you have expected an error, for example?
...
- You might have been surprised that the inital code did not result in an error,
  ...
  (This leads to the realization that you don't _have to_ map aesthetics to
  dataframe columns,
  you can also provide stand-alone vectors:
  you'll see more of that in the next exercise.)
...
Next, ggplot is able to map this vector to the color aesthetic in the plot,
