...
- _tidyverse_ -- As metapackage that includes _ggplot2_ which we'll use for
...
_Also, instead of copying-and-pasting code,_
_you could download this [R script - TBA](TBA) with today's code._
...
- _Non-metric Multidimensional Scaling_ (**nMDS**)
  ...
  that is especially suitable when you have low confidence in distance values.
...
(Like last week's `aov()` function, these are in the *stats* package,
...
PC1, the first principal component, explains a whopping 68.8% of the variation,
whereas PC4 explains merely 2.7% of the variation in the dataset.
(Principal components are always ordered by the amount of variation they explain,
...
Running the `str()` function is a good start for getting the raw contents of
the object,
...
Either download this [R script - TBA](TBA), open it in RStudio, and run the code,
or:
...
- **`pca$sdev`** is a vector of standard deviations
  ...
  (This is also known as the _eigenvector_, which contains _eigenvalues_.)
  We also saw this information when running `summary(pca)`.
...
- **`pca$rotation`** is a matrix that contains the **loadings** for each variable
  ...
  with _higher absolute values_ indicating a larger influence of the variable
  on the PC.
  The _sign_ (- or +) matters too:
  in PC1, larger values of bill depth lower the PC value,
...
- **`pca$x`** is the most-used part of the output:
  ...
  for each PC, used to create a score plot.
    ```{r}
    head(pca$x)
    ```
...
is a barplot that shows
the **amount of variation (_eigenvalues_) explained by each PC.**
...
#### Interpretation
This gives us a quick visual overview of what we saw earlier:
PC1 is _by far_ the most important, and PC4 doesn't do much at all.
PC3 may still be worth plotting but "luckily" we have a decent gap between PC2
and PC3, such that a plot of the first two PCs will be a very logical choice.
...
A "score plot" shows **the scores (coordinates) for each sample for two PCs**,
typically the first two.

We're going to need a dataframe to plot.
But if we were to `broom::tidy()` the matrix with scores (`pca$x`) like we've
done with t-test and ANOVA output in previous weeks,
...
which will allow us to color points by, say, `species`.
...
we will use the `tidy()` function like in previous weeks.
Because the output of `prcomp()` contains multiple matrices,
we'll have to point it to the eigenvalues using the `matrix` argument
...
Finally, we can modify the aspect ratio, which is `height / width` --
...
which has a convenient function for making biplots, `fviz_pca_biplot()`:
...
there are also _factoextra_ functions for these and other PCA plots:
...
## just to improve the visualization,
## we use a multiplication factor:
mult <- 2.5
...
Above, we plotted the scores for the first two PCs in our
score plot and biplot.
Now, _create a biplot with another combination of PCSs_
(e.g., PC3 & PC4, or PC1 & PC3).

Take a look at the help for the `fviz_pca_biplot()` function by typing  
`?fviz_pca_biplot` to find out how you might be able to plot different PCs.

<details><summary><b>Hints</b> (click here)</summary>
<br>

The `axes` argument to `fviz_pca_biplot()` controls which axes will be plotted;
...
in which you color by something else than `species`, e.g. by `sex`.
(If you want, also make a scree plot and/or a score plot.)

<details><summary><b>Hints</b> (click here)</summary>
<br>

- Use the _dplyr_ function `filter()` on `penguins_noNA` object
