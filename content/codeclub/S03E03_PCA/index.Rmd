...
But because the output of `prcomp()` contains multiple matrices,
we'll have to point `tidy()` to the eigenvalues using the `matrix` argument
...
Finally, we can modify the aspect ratio,
which is expressed as `height / width` --
...
there are also _factoextra_ functions for these for and other PCA plots:
...
(e.g., PC1 & PC3, or PC3 & PC4 -- whatever you think will be more informative).
