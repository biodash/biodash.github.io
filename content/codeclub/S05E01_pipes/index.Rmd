...
To give a very simple (but therefore not very practical) example:
without a pipe, we can print the number of rows in the `diamonds` dataframe
using the `nrow()` function with `diamonds` as the sole argument:

```{r}
nrow(diamonds)
```

Instead, we could also take the diamonds dataframe and then pipe (`|>`) it
...
**subset the `diamonds` dataframe to only show the columns**
**`color`, `depth`, and `price`**
