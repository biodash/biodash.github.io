...
Mike S. will teach us about functionals.**

In R, *functionals* like the `apply` and `map` are often preferred over loops
...
inside the loop, simply print each number to screen.
...
```{r}
for (bill_len in penguins$bill_depth_mm[1:8]) {
  print(bill_len + 100)
}
```
...
- Creating a single R object (e.g. a dataframe) across the entire loop:
...
We'll also take this opportunity to explicitly see how we change a bit of code
that does something once in order to use it in a loop.
...
Note that the code above would be exactly the same regardless of whether we had
...
for (focal_species in all_penguin_species) {
  ...
  write_tsv(one_penguin_df, filename)
}
```
...
Inside the parentheses `()` after `if` should be a test that evaluates to either
`TRUE` or `FALSE`.
