...
**Making data frames from map outlines by ggplot2** <br/>
Recall that ggplot2 operates on data frames. Therefore, we need some way to translate the maps data into a data frame format the ggplot can use. The package *ggplot2* provides the `map_data()` function.
...
ohio <- filter(states, region == "ohio")
```
...
ohio_indiana <- filter(states, region == "ohio" | region == "indiana")
...
- The points are plotted on the boundaries of the counties
