...
```{r, eval=FALSE}
desserts <- c("plum pudding", "chocolate cake", "sticky toffee pudding")
...
```{r, eval=FALSE}
str_view(desserts, "pudding", match = TRUE)
```
...
Unfortunately, we have to wade through some potentially confusing territory
before we can start using regular expressions.
...
You may already see the problems that arise here:
how, then, do we indicate a literal **`.`** or **`\`** in a regular expression?
The solution is to **escape** the special character with a(nother) backslash:
...
What if we want to match a backslash?
...
which are about The Great British Bake Off (GBBO) television show.
...
so we'll also start try to matching them with regular expressions.
...
Other metacharacters, that is, characters that represent a single instance of
**a character type**, are actually character combinations starting with a **`\`**.
...
(These include **`\n`** and **`\t`**,
which mean the same in non-regex escape sequences in R.)
...
- What about periods? Note that we need to escape the period with two **`\\`**.
  ...
  ```{r, eval=FALSE}
  str_view_all(signatures, "\\.", match = TRUE)
  ```

  ```{r, echo=FALSE, results="show", out.width="50%", fig.align="left"}
...
- Match the entire string (full names -- by flanking the pattern with **`.*`**)
  of names with 2 to 3 (**`{2,3}`**) consecutive "*e*" characters:
...
- Match all three-letter names --first or last--
  by matching three word-characters (**`\w`**) surrounded by word-boundaries
  (**`\b`**):
