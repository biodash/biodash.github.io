...
lowell_tokens <- lowell %>% 
    str_to_lower() %>%               # get rid of upper case; returns a character vector.
    str_extract_all("[a-z]+") %>%    # get rid of punctuation; returns a list.
    unlist() %>%                     # flatten that list
    as_tibble()                      # stick it in a data frame
...
Punctuation has been stripped and all words are lower case, but possessives and contractions are preserved (fancy usage of str_ grep functions under the hood...).
