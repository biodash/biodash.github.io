...
and turn values smaller than 5 into NA, while leaving the other values unchanged:
...
To do that,
we can first create a new variable that indicates whether a value for another
variable is  missing or not.
Then, we can map an aesthetic like `color` to this missing-or-not variable,
...
- Side note: An alternative way of making this kind of plot would be with
  `cut_width()` instead of `cut_number()`:
...
To better understand the relationship between `color`, `carat`, and `price`
