...
We'll also have a quick look at the `flights` dataset,
...
It can be useful to compare distributions among observations with and
without missing values.
To do that,
we can first create a new variable that indicates whether a value is
missing or not.
Then, we can map an aesthetic like `color` to this variable to show the 
two groups separately.

Below, we'll compare flights with and without missing values for departure time
(`dep_time`), since the former are cancelled flights,
...
Or between the cut and color of a diamond (two categorical variables)?
Or between the carat and the price of a diamond (two continuous variables)?

### 7.5.1: A categorical and continuous variable

In out last plot above,
we actually explored the relationship between a categorical variable
...
whether prices differ among diamond cuts:
...
Another classic way of showing the relationship between a categorical and a
continuous variable is with a **boxplot**.
...
Let's make a boxplot of diamond prices by cut:
...
A slightly more visually appealing way to plot this is using a heatmap,
which we can do with `geom_tile()` after we calculate the counts ourselves:
...
most intuitive of the combinations.
...
with some areas of solid black --
though this type of thing can get a lot worse and the overall pattern is still
clear in this case.
...
- Bin one of the continuous variables,
...
_Tip: If you can't read the x-axis labels (bin names) in the resulting plot _
...
- You can start by creating a binned column with `mutate()` and
  ...
  by simply using `cut_width(carat, 0.1)` as `x` or `y` aesthetic.

- An alternative way of making this kind of plot would be by using `cut_width()`
  instead of `cut_number()`:
  then, you fix the width of each bin rather than the number of data points in
  each bin.
  The disadvantage of that approach is that you may be misled by bins with
  very few data points, but a way to ameliorate this is by using the
  `varwidth = TRUE` argument of `geom_boxplot()`.
...
```{r}
diamonds %>%
  ggplot(mapping = aes(y = cut_number(carat, 10), x = price)) + 
  geom_boxplot() +
  labs(y = "carat (binned)")
```

The book has the following code to do this,
which is less intuitive but has the advantage of keeping the carat axis labels
as if it still were a continuous variable:
...
Alternatively, you can use the `cut_width()` function:

```{r}
diamonds %>%
  ggplot(mapping = aes(y = cut_width(carat, 0.2), x = price)) + 
  geom_boxplot() +
  labs(y = "carat (binned)")
```

Use `varwidth = TRUE` if you want the width of the boxplots to reflect the number
of data points:
...
- Create a heatmap (`geom_tile()`) that shows the mean diamond price for
  each diamond color and cut combination.

- From your heatmap,
  would you say that going from color `D` => `J` is associated with an _increase_
  or a _decrease_ in the mean price?

_Tip: add `+ scale_fill_viridis_c()` at the end for a much nicer color scale._

<details><summary><b>Hints</b> (click here)</summary>
<br>

- In the heatmap, you'll want `color` along the `x` axis and `cut` along the `y`
  axis (or vice versa), and you'll want to `fill` the tiles by price.
...
It looks like going from color `D` to `J` is associated with an overall increase
in the mean price.
...
- Bonus: to better understand the relationship between `color`, `carat`, and `price`,
  modify the earlier scatterplot of `carat` and `price` simply by mapping diamond
  `color` to the color aesthetic.
...
- A simple way of taking carat into account is by using price-per-carat rather
  than price in your heatmap.
...
- Since `carat` is strongly positively associated with `price`,
  it is therefore not fair to compare prices among colors without controlling for
  `carat`.
...
diamonds %>%
  ...
  ggplot(mapping = aes(x = color, y = cut)) +
  geom_tile(mapping = aes(fill = price_per_carat)) +
  scale_fill_viridis_c()
```
...
- A scatterplot of carat and price that includes diamond color confirms this
  pattern:
...
function (the same code would work if `cut` would not yet have been a factor):
