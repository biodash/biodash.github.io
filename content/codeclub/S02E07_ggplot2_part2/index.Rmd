...
we may want to add the raw data points to our boxplot:
...
We had previously set the aesthetics inside the `geom_boxplot()` call --
that is, we set it for that geom only, and not for the entire plot.
...
- Specify the `aes()` _also_ inside `geom_point()`.
...
them.
Therefore, even for daily users _ggplot2_, creating a publication-ready figure
will usually involve some Googling or checking the
