...
- It's a very fast aligner, but memory-intensive. Luckily we have the OSC!
...
- STAR is available in a module at OSC. The Owens cluster has a more recent
  version available, so make sure to log in to the Owens cluster (and not Pitzer).
...
According to the documentation, the value of `genomeSAindexNbases` should be
log2(GenomeLength)/2 - 1) => log2(782520133)/2 - 1) = 13.77 = 14

.content-box-warning[
I have not looked into the different options much and am using the same
options that Matthew specified, here.

Before running this on new data, should be worth looking at the STAR documentation
to check the options are appropriate.

(E.g. check the `` option referenced
...
.content-box-warning[
Check out max intron size in GFF
