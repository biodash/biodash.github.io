...
birds_file_in <- 'assets/data/birds/backyard-birds_sample_orig.tsv'
birds_file_out <- 'assets/data/birds/backyard-birds_sample.tsv'

birds_ohio_file <- 'assets/data/birds/backyard-birds_Ohio_orig.tsv'
birds_ohio <- read_tsv(birds_ohio_file)
...
birds <- read_tsv(birds_file_in, col_names = colnames(birds_ohio)) %>%
...
birds <- birds[tolower(birds$stateProvince) %in% unique(states$region), ]

# Check and write -----------------------------------------

# Check if it worked:
#birds %>% filter(is.na(species_en)) %>% pull(species) %>% unique()

# Write birds file:
write_tsv(birds, birds_file_out)

# Create errors in lat-lon for exercises -----------------------
birds_error <- birds

caro_nc <- birds_error$species_en == "Carolina Chickadee" & birds_error$stateProvince == 'North Carolina'
caro <- birds_error$species_en == "Carolina Chickadee"

birds_error$lat[caro] <- -birds_error$lat[caro]
birds_error$long[caro_nc] <- birds$long[caro_nc] + 10

birds_error_file_out <- 'assets/data/birds/backyard-birds_sample_error.tsv'
write_tsv(birds_error, birds_error_file_out)
